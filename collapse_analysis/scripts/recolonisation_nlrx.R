#Recolonisation experiment - how long does it take for a colony to re-establish after it has been completely lost.
...
outpath <- file.path("D:/seabirdExtirpation/output/recolonisation")

# Setup nl object - this initialises an instance of NetLogo
recol_nl <- nl(nlversion = "6.2.2",
                 nlpath = netlogopath,
                 modelpath = modelpath,
                 jvmmem = 1024)
...
default_ls <- list(
  ...
  "behav-output-path" = "\"../output/recolonisation/\""
)

#Creating a vector of all the initialisation files to be iterated over.
init_files <- list.files("../data/recolonisation/", full.names = TRUE) %>%
...
#Bench marking
tic("recolonisation experiment loop")
...
param_length <- length(recol_ls[[1]]$values)

#Setting up the nlrx experiment
recol_nl@experiment <- experiment(
  expname = paste0("recol_", variable_name),
  outpath = "./",
  repetition = 1,
  tickmetrics = "false",
  idsetup = c("setup"),
...
recol_nl@simdesign <- simdesign_distinct(nl = recol_nl,
                                           nseeds = 100)

#Pre-flight checks...
#print(recol_nl)
#eval_variables_constants(recol_nl)
...
recol_results <- progressr::with_progress(
  run_nl_all(recol_nl,
             split = ncores)
)
...
#saveRDS(recol_results, "./output/recolence/recol_meta_results.rds")
