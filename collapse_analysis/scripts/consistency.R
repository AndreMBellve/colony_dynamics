...
#Took approximately 9496 seconds (2.6 hours to run)
...
adult_var_df <- twoIsl_df |> 
  ...
  pivot_longer(cols = starts_with("settled_"),
               names_to = "island_id",
               values_to = "adult_count") |> 
  select(c(run, island_id, adult_count)) |>
  arrange(run) %>% 
  group_by(island_id) |> 
  mutate(adult_mean = cummean(adult_count),
         adult_sd = cumvar(adult_count, sd = TRUE))
...
ggplot(adult_var_df, aes(run, adult_mean,
                         group = island_id,
                         colour = island_id)) +
  geom_line(alpha = 0.5) +
  theme(axis.text.x = element_text(angle = 45))
...
ggplot(adult_var_df, aes(run, adult_sd,
                         group = island_id,
                         colour = island_id)) +
  geom_line(alpha = 0.5) +
  theme(axis.text.x = element_text(angle = 45))
